...
static int mcp3221_read_async(const struct device *dev, const struct adc_sequence *sequence, struct k_poll_signal *async)
{
	struct mcp3221_data *data = dev->data;
	int err;
...
static int mcp3221_read_channel(const struct device *dev, uint8_t channel, uint16_t *result)
{
...
static int mcp3221_channel_setup(const struct device *dev, const struct adc_channel_cfg *channel_cfg)
{
...
static const struct adc_driver_api mcp3221_adc_api = {
...
static void mcp3221_acquisition_thread(void *p1, void *p2, void *p3)
{
	...
	while (true) {
		...
		err = mcp3221_read_channel(data->dev, 0, &result);
		if (err) {
...
static int mcp3221_init(const struct device *dev)
{
	...
	k_tid_t tid =
		k_thread_create(&data->thread, data->stack, K_KERNEL_STACK_SIZEOF(data->stack), mcp3221_acquisition_thread,
				data, NULL, NULL, CONFIG_ADC_MCP3221_ACQUISITION_THREAD_PRIO, 0, K_NO_WAIT);
...
#define MCP3221_DEVICE(n)                                                                                                   \
	...
	DEVICE_DT_INST_DEFINE(n, &mcp3221_init, NULL, &mcp3221_data_##n, &mcp3221_config_##n, POST_KERNEL,                  \
			      CONFIG_ADC_INIT_PRIORITY, &mcp3221_adc_api)
